...
multiple_query(FunL, DataL) ->
    master_server ! {slaves_request, {identity, self()}, {quantity, lists:length(FunL)}},
    receive
        {slaves_id, ListSlaves} ->
            [Pid ! {compute_request, {identity, self()}, F, D} || F <- FunL, D <- DataL, Pid <- ListSlaves]
    end,
    [receive
        {compute_result, {identity, From}, {value, V}} -> V
    end || From <- ListSlaves].
